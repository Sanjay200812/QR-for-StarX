...
export const SocialCard: React.FC<SocialCardProps> = ({
  ...
  const content = (
    <>
      ...
      <div className="flex items-center space-x-3.5 sm:space-x-4 min-w-0 pr-2">
        ...
        <div className="flex flex-col text-left min-w-0">
          <div className="flex items-center space-x-2">
            <span className="text-white font-semibold text-[15px] sm:text-base tracking-tight leading-snug group-hover:text-white transition-colors truncate">
              {title}
            </span>
            {badge && (
              <span className="text-[10px] font-semibold tracking-wider uppercase px-2 py-0.5 rounded-full bg-accent/20 text-accent border border-accent/30">
                {badge}
              </span>
            )}
          </div>
          <span className="text-text-secondary text-[13px] sm:text-sm font-normal tracking-tight truncate max-w-[200px] sm:max-w-[240px] leading-tight mt-0.5">
            {subtitle}
          </span>
        </div>
      </div>
      ...
      {/* Right Chevron Icon */}
      <div className="shrink-0 flex items-center justify-center w-8 h-8 rounded-full bg-white/[0.03] border border-white/[0.06] text-text-muted group-hover:text-accent group-hover:border-accent/30 transition-all duration-200">
